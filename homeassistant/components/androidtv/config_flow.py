...
class AndroidTVFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if CONF_ADB_SERVER_IP in user_input and adb_key:
            return self._show_setup_form(user_input, "key_and_server")

        if CONF_ADB_SERVER_IP not in user_input:
            user_input.pop(CONF_ADB_SERVER_PORT, None)

        if adb_key and not await self.hass.async_add_executor_job(_is_file, adb_key):
